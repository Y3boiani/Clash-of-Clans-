...
import {
  ...
  CoordinationHeatmap,
  MiniStatViz
} from '@/components/MLVisualizations'

const API = `${process.env.NEXT_PUBLIC_BACKEND_URL}/api`
const DEFAULT_CLAN = '#2G8VUQGP8'

function UnifiedDashboardContent() {
  const searchParams = useSearchParams()
  const clanTag = searchParams.get('clan') || DEFAULT_CLAN
  ...
  const loadAllAnalyses = async () => {
    ...
    try {
      const statsResponse = await axios.get(
        `${API}/data/clan/${encodeURIComponent(clanTag)}/stats`
      )
      setDataStats(statsResponse.data)

      const analysisPromises = {
        leadership: axios.post(`${API}/ml/leadership/analyze`, { clan_tag: clanTag }).catch(e => ({ error: e.message })),
        donations: axios.post(`${API}/ml/donations/analyze`, { clan_tag: clanTag }).catch(e => ({ error: e.message })),
        capital: axios.post(`${API}/ml/capital/analyze`, { clan_tag: clanTag }).catch(e => ({ error: e.message })),
        fairness: axios.post(`${API}/ml/fairness/audit`, { clan_tag: clanTag }).catch(e => ({ error: e.message })),
      }

      const results = await Promise.all(Object.values(analysisPromises))
      const keys = Object.keys(analysisPromises)
      
      const analysesData: any = {}
      keys.forEach((key, index) => {
        analysesData[key] = (results[index] as any).data || results[index]
      })
    ...
    return (
      <div className="min-h-screen bg-coc-dark coc-wood-bg flex items-center justify-center">
        <div className="text-center">
          ...
          <div className="text-yellow-200 text-sm mt-2">Running 7 ML models on your clan data</div>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-coc-dark coc-wood-bg">
      <div className="max-w-7xl mx-auto px-4 py-8">
        {/* Header */}
        <div className="mb-8">
          <Link href="/" className="text-coc-gold hover:text-yellow-300 mb-4 inline-block font-semibold">
            ← Return to Base
          </Link>
          <h1 className="text-4xl md:text-5xl font-bold text-coc-gold mb-2 gold-shine uppercase">
          ...
          <div className="coc-card p-6 mb-8 border-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              <div className="coc-stat-box">
                <div className="text-3xl md:text-4xl font-bold text-coc-gold mb-1">{dataStats.data_availability.player_snapshots}</div>
                <div className="text-xs text-yellow-200 uppercase">Warriors</div>
              </div>
              <div className="coc-stat-box">
                <div className="text-3xl md:text-4xl font-bold text-orange-400 mb-1">{dataStats.data_availability.wars}</div>
                <div className="text-xs text-yellow-200 uppercase">Battles</div>
              </div>
              <div className="coc-stat-box">
                <div className="text-3xl md:text-4xl font-bold text-red-400 mb-1">{dataStats.data_availability.war_attacks}</div>
                <div className="text-xs text-yellow-200 uppercase">Attacks</div>
              </div>
              <div className="coc-stat-box">
                <div className="text-3xl md:text-4xl font-bold text-purple-400 mb-1">{dataStats.data_availability.capital_raids}</div>
                <div className="text-xs text-yellow-200 uppercase">Raids</div>
              </div>
              <div className="coc-stat-box">
                <div className="text-3xl md:text-4xl font-bold text-green-400 mb-1">{dataStats.data_availability.days_of_data}</div>
        ...
        <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
          {tabs.map((tab) => (
            <button
              ...
              className={`coc-tab px-4 md:px-6 py-3 rounded-t-lg font-bold transition-all whitespace-nowrap ${
...
// Overview Tab
function OverviewTab({ analyses }: { analyses: any }) {
  return (
          ...
          <table className="coc-table">
            <thead>
              <tr>
                ...
                <th className="p-3">Influence Power</th>
                <th className="p-3">Role</th>
              </tr>
            </thead>
            <tbody>
              {data.top_leaders?.slice(0, 10).map((leader: any) => (
                <tr key={leader.rank}>
                  <td className="p-3">
...
// Donations Tab
function DonationsTab({ data }: { data: any }) {
  if (!data || data.error) {
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="coc-card p-6 border-4">
          ...
          <div className="space-y-3">
            {data.top_contributors?.slice(0, 5).map((contributor: any, i: number) => (
...
// Capital Tab
function CapitalTab({ data }: { data: any }) {
  if (!data || data.error) {
...
// Fairness Tab
function FairnessTab({ data }: { data: any }) {
  if (!data || data.error) {
...
async def analyze_leadership(request):
  ...
  return (
    <div className="space-y-6">
      <div className="coc-card p-6 border-4">
        <h2 className="text-2xl font-bold text-coc-gold mb-4">🔧 Backend Architecture & Workflow</h2>
      ...
      {workflows.map((workflow) => (
        <div key={workflow.step} className="coc-card p-6 border-4">
          <div className="flex items-start gap-4">
            <div className="coc-badge w-10 h-10 flex-shrink-0">
              <span className="text-coc-gold font-bold">{workflow.step}</span>
            </div>
            <div className="flex-1">
              ...
              <p className="text-yellow-200 mt-2 mb-3">{workflow.description}</p>
      ...
      <div className="coc-card p-6 border-4">
        <h3 className="text-xl font-bold text-coc-gold mb-4">📁 Backend File Structure</h3>
